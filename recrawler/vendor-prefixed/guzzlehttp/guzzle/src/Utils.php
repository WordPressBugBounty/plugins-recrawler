...
use Mihdan\ReCrawler\Dependencies\GuzzleHttp\Handler\CurlHandler;
use Mihdan\ReCrawler\Dependencies\GuzzleHttp\Handler\CurlMultiHandler;
...
use Mihdan\ReCrawler\Dependencies\GuzzleHttp\Handler\Proxy;
use Mihdan\ReCrawler\Dependencies\GuzzleHttp\Handler\StreamHandler;
...
EOT
    ...
    {
        \Mihdan\ReCrawler\Dependencies\trigger_deprecation('guzzlehttp/guzzle', '7.15', '%s() is deprecated and will be removed in 8.0. Use PHP\'s json_decode() instead.', __METHOD__);
        if ($depth < 1) {
    ...
    public static function normalizeIdnConversionOption($value) : ?int
    {
        ...
        if (\is_string($value) && \is_numeric($value) || \is_float($value) && \is_finite($value)) {
            \Mihdan\ReCrawler\Dependencies\trigger_deprecation('guzzlehttp/guzzle', '7.11', 'Passing %s as the "idn_conversion" request option is deprecated; guzzlehttp/guzzle 8.0 will reject values that are not true, false, null, or an integer IDNA_* bitmask.', \get_debug_type($value));
            return (int) $value;
        }
