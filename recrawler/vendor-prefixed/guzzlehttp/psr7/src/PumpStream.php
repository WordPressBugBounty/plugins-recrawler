...
final class PumpStream implements StreamInterface
{
    ...
    public function seek($offset, $whence = \SEEK_SET) : void
    {
        ...
        if (!\is_int($whence)) {
            \Mihdan\ReCrawler\Dependencies\trigger_deprecation('guzzlehttp/psr7', '2.11', 'Passing %s to StreamInterface::seek() is deprecated; guzzlehttp/psr7 3.0 requires int for $whence.', \get_debug_type($whence));
        }
    ...
    public function getMetadata($key = null)
    {
        if ($key !== null && !\is_string($key)) {
            \Mihdan\ReCrawler\Dependencies\trigger_deprecation('guzzlehttp/psr7', '2.11', 'Passing %s to StreamInterface::getMetadata() is deprecated; guzzlehttp/psr7 3.0 requires string|null for $key.', \get_debug_type($key));
        }
        if (!$key) {
