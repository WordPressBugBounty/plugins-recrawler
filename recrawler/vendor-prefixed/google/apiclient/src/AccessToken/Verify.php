 ...
 */
namespace Mihdan\ReCrawler\Dependencies\Google\AccessToken;

use DateTime;
use DomainException;
use Exception;
use Mihdan\ReCrawler\Dependencies\ExpiredException;
use Mihdan\ReCrawler\Dependencies\Firebase\JWT\ExpiredException as ExpiredExceptionV3;
...
use Mihdan\ReCrawler\Dependencies\Google\Exception as GoogleException;
use Mihdan\ReCrawler\Dependencies\GuzzleHttp\Client;
use Mihdan\ReCrawler\Dependencies\GuzzleHttp\ClientInterface;
use InvalidArgumentException;
use LogicException;
use Mihdan\ReCrawler\Dependencies\phpseclib3\Crypt\AES;
...
class Verify
{
    ...
    private function setPhpsecConstants()
    {
        if (\filter_var(\getenv('GAE_VM'), \FILTER_VALIDATE_BOOLEAN)) {
            if (!\defined('Mihdan\\ReCrawler\\Dependencies\\MATH_BIGINTEGER_OPENSSL_ENABLED')) {
                \define('Mihdan\\ReCrawler\\Dependencies\\MATH_BIGINTEGER_OPENSSL_ENABLED', \true);
            }
            if (!\defined('Mihdan\\ReCrawler\\Dependencies\\CRYPT_RSA_MODE')) {
                \define('Mihdan\\ReCrawler\\Dependencies\\CRYPT_RSA_MODE', AES::ENGINE_OPENSSL);
            }
        }
    }
}
